...
while L<=length:
    ...
    def velocity_loss_sin(height, blockage_value, ridge_width, density, viscosity):
        ...
        if x<(ridge_width/2):
            ...
            v_f_sin=(v-c*v)/(c+1) #-((c+1)*v)/(c-1)
    def v_wall_sin(v_f_sin, Re, height, blockage_value):
        global v_use_wall
        n=3.5
        F = 0.316 * (1 / Re) ** (0.25)
        if x < (ridge_width / 2):
            v_use_wall = (v_f_sin * (1 + 1.33 * (F) ** 0.5)) * ((1-((height[(int(x / x_step))] - blockage_value[(int(x / x_step))])/0.015))**(1/n)) #5.3
        else:
            v_use_wall = (v_f_sin * (1 + 1.33 * (F) ** 0.5)) * (
                        (1 - ((height[(int(x / x_step))-int(x_size/2)] - blockage_value[(int(x / x_step))-int(x_size/2)]) / 0.015)) ** (
                            1 / n))  # 5.3
    x_size = 100
    ...
    while x <= (ridge_width / 2):
        ...
        #v_wall_sin(v_f_sin, Re, height, blockage_value)
        v=v_f_sin
        """f.append((Sr*v_use_wall) / (frequency))
        l.append(L)
        v_data.append(v)
    ...
    while x <= (ridge_width):
        ...
        #v_wall_sin(v_f_sin, Re, height_down, blockage_value_inverse)
        v = v_f_sin
...
outData.to_excel("Output/Frequenzen5.xlsx")
plt.plot(l, f)
plt.xlabel("$L$[m]")
plt.ylabel("$f$[Hz]")

...
for r in r_value:
    ...
    while o <= 50:
        ...
        if len(omega_current) != 0 and len(omega_current2) != 0:
            break
        o = o + 0.01
outData = pd.DataFrame(outData, columns=["R[m]", "f[Hz]"])
outData2 = pd.DataFrame(outData2, columns=["R[m]", "f[Hz]"])
outData.to_excel("Output/Radius.xlsx")
outData2.to_excel("Output/Radius2.xlsx")
print(omega_value)
print(r_use)
plt.plot(r_use, omega_value)
plt.plot(r_use2, omega_value2)
plt.title('Radius')
plt.show()
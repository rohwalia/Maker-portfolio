    ...
    while o <= 50:
        ...
        while L <= length:
            ...
            while x <= (ridge_width / 2):
                blockage=height[int(x / x_step)]-R
        ...
        if len(omega_current) != 0 and len(omega_current2) != 0:
            break
        o = o + 0.01
outData = pd.DataFrame(outData, columns=["L[m]", "f[Hz]"])
outData2 = pd.DataFrame(outData2, columns=["L[m]", "f[Hz]"])
outData.to_excel("Output/Length.xlsx")
outData2.to_excel("Output/Length2.xlsx")
print(omega_value)
print(l_use)
plt.plot(l_use, omega_value)
plt.plot(l_use2, omega_value2)
plt.title('Length')
plt.show()